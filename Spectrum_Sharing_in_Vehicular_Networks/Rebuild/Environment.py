...
class V2VChannels:
    ...
    def update_shadow(self, delta_distance):
        if self.Shadow is None:
            self.Shadow = np.random.normal(0, self.shadow_std, size=(self.n_veh, self.n_veh))
        else:
            delta2 = np.zeros((self.n_veh, self.n_veh))
            for i in range(self.n_veh):
                for j in range(self.n_veh):
                    delta2[i][j] = delta_distance[i] + delta_distance[j]
            self.Shadow = np.exp(-1 * (delta2 / self.decorrelation_distance)) * self.Shadow + np.sqrt(
...
class Environment:
    ...
    def __init__(self, M, K, B):
        ...
        self.is_active = np.ones((self.n_veh, self.n_des))
    ...
    def update_large_fading(self, positions, time_step):
        ...
        self.V2IChannels.update_shadow(self.delta_distance)
        self.V2VChannels.update_shadow(self.delta_distance)
    ...
    def test(self):
        ...
        v2i.update_pathLoss()
        v2i.update_shadow(self.delta_distance)
    ...
    def init_all_vehicles(self):
        ...
        self.delta_distance = np.array([c.velocity * 0.1 for c in self.vehicles])
    ...
    def renew_environment(self):
        ...
        # print('V2V 信道情况', self.V2V_channels_abs.mean())
        # print('车辆位置',[c.position for c in self.vehicles])
        # print('V2V shadow', self.V2VChannels.Shadow.mean())
    ...
    def compute_V2V_interference(self, all_actions):
        V2V_interference = np.zeros((self.n_veh, self.n_des, self.n_sub_carrier)) + self.sig2
        ...
        #         )
        #         for k in range(t + 1, len(indexes)):
        #             other_i, other_j = indexes[k]
        #             V2V_interference[i, j, m] += power_dB2W(
        #                 self.V2V_power_dB_list[all_actions[other_i, other_j, 1]]
        #                 + self.interference_dB_from_other_V2V((i, j), (other_i, other_j), m)
        #             )
        #             V2V_interference[other_i, other_j, m] += power_dB2W(
        #                 self.V2V_power_dB_list[all_actions[i, j, 1]]
        #                 + self.interference_dB_from_other_V2V((other_i, other_j), (i, j), m)
        #             )
        for m in range(self.n_sub_carrier):
            for i in range(self.n_veh):
    ...
    def get_observations(self, all_actions):
        payloads = self.remain_payload / self.payload_size
        remain_time = self.remain_time / self.action_time_step
    ...
    def get_V2V_capacity(self, all_actions, update_payload=True):
        channels = all_actions[:, :, 0]
        channels[(np.logical_not(self.is_active))] = -1
        power_selection = all_actions[:, :, 1]
        V2V_signal = np.zeros((self.n_veh, self.n_des))
        V2V_interference = np.zeros((self.n_veh, self.n_des))
        for m in range(self.n_sub_carrier):
            indexes = np.argwhere(channels == m)
            for a in range(len(indexes)):
                i, j = indexes[a]
                V2V_signal[i, j] = power_dB2W(self.V2V_power_dB_list[power_selection[i, j]]
                                              + self.gain_dB_of_V2V((i, j), m))
                V2V_interference[i, j] += power_dB2W(
                    self.V2I_power_dB + self.interference_dB_from_V2I_to_V2V((i, j), m))
                for b in range(a + 1, len(indexes)):
                    oi, oj = indexes[b]
                    V2V_interference[i, j] += power_dB2W(
                        self.V2V_power_dB_list[power_selection[oi, oj]]
                        + self.interference_dB_from_other_V2V((i, j), (oi, oj), m)
                    )
                    V2V_interference[oi, oj] += power_dB2W(
                        self.V2V_power_dB_list[power_selection[i, j]]
                        + self.interference_dB_from_other_V2V((oi, oj), (i, j), m)
                    )
        V2V_interference += self.sig2
        V2V_capacity = self.bandwidth * np.log2(1 + V2V_signal / V2V_interference)
        self.remain_payload -= (V2V_capacity * self.action_time_step).astype('int32')
        self.remain_payload[self.remain_payload < 0] = 0
        self.is_active[self.remain_payload <= 0] = 0
