...
class Algorithm:
    ...
    def save_model(self, model_root_dir):
        ...
        np.save(os.path.join(model_dir, 'loss.npy'), np.asarray(self.loss_list))
        np.save(os.path.join(model_dir, 'reward.npy'), np.asarray(self.reward_list))
        np.save(os.path.join(model_dir, 'p.npy'), np.asarray(self.p_list))
        np.save(os.path.join(model_dir, 'V2I_rate.npy'), np.asarray(self.V2I_rate_list))
...
if __name__ == '__main__':
    ...
    algorithm.save_model('./model v4.2')
    algorithm.show()
